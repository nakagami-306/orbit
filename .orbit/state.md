<!-- orbit:generated | 2026-05-03T10:04:19Z | branch:main | head:019ded4b-c78c-726f-b0d4-2ad6bec6f4a1 -->
# orbit
...
`.orbit/` ディレクトリは **git track 対象外**（`.gitignore` に `.orbit/` を追加）。理由は以下:

- **config.toml の project_id は per-user の中央 DB (~/.orbit/orbit.db) を指すローカルポインタ**である。Orbit は現状 multi-user sync 機構を持たないため、他者が clone しても同じ project_id の entity を解決できず共有価値がない。
- **state.md は p_sections からの再生成物**であり、Section 編集のたびに差分が出る。track するとコミットノイズとマージコンフリクトの温床になる。derived artifact は `*.db` と同じ扱いにする。

将来 multi-user sync を導入する際は、config.toml だけ track 対象に戻して「リポジトリ ↔ 共有プロジェクト」のリンクを表現する余地がある。state.md は引き続き derived artifact として ignore する。
...
- Stateビュー — Section一覧をposition順で表示し、各sectionをクリックで content（マークダウン全文）をinline展開。`/api/projects/:id/sections` から SectionSummary（id/title/content/position/isStale/staleReason）を取得して保持
...
listen address は 0.0.0.0:<port> で同一LAN内デバイスからアクセス可能（スマホ等の手元端末からの参照用途）。認証機構は持たないため、共有Wi-Fi下や信頼できないネットワークでは使用しない前提。専有開発機での運用に限定する設計判断。
