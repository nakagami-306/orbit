<!-- orbit:generated | 2026-04-26T05:00:13Z | branch:main | head:019dc828-d779-7497-bfb2-2071ecde5112 -->
# orbit
...
- Entry記録のタイミング/型選択など曖昧な領域は、SessionStartで明示的なルールを伝え強制はしない（過去の強制が失敗しているため）
- 記録自体の摩擦を下げる方向（orbit thread add の type推論等）で自然な記録発生を狙う（Phase 2予定）
...
orbit-decide-guard.py の判定はすべて regex/構造ベース（軽量）。AIに判定させないため、廃止前のStop hookで問題だった精度低下は構造的に発生しない。

Stop hookの再導入（議論記録漏れの軽量警告）はPhase 2の課題として残っている。判定はノンブロッキング・nudge型で設計する予定（過去の二重警告問題を回避）。
...
Claude Code #27398 によりplugin同梱のhooks.jsonは自動検出されない（2026-04時点）。回避策として orbit init はプロジェクトの .claude/settings.json にhooks設定を自動生成し、各hookコマンド内でinstalled_plugins.json経由でplugin cache内のスクリプトをexecで読み込む。これにより plugin更新（marketplace update + reload-plugins）への追従性は維持される。
...
Plugin v2 のPhase 1（SessionStart刷新・DecideGuard強化・AI連携Section再構成）は2026-04に完了。Phase 2 として以下を予定:

- Stop hook新設（議論記録漏れの軽量ノンブロッキング警告）
- orbit thread add の type推論・直近assistant出力からのEntry化（記録摩擦削減）
- orbit decide のCLI出力にTask提案ステップを追加（Decide→Task連鎖の補強）
