...
import { fetchJSON, type DAGResponse, type ProjectDetail, type EntityNode } from '../api/client'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '8px' }}>
          <Link to="/" style={{ color: '#888', textDecoration: 'none', fontSize: '0.8rem' }}>
            Projects
          </Link>
        ...
        {project && (
          <div style={{ fontSize: '0.75rem', color: '#888', display: 'flex', gap: '1rem', marginBottom: '12px' }}>
            ...
            {project.openThreads > 0 && (
              <span style={{ color: '#fa4' }}>{project.openThreads} open threads</span>
            )}
            {project.unresolvedConflicts > 0 && (
              <span style={{ color: '#f44' }}>{project.unresolvedConflicts} conflicts</span>
            )}
            {project.staleSections > 0 && (
              <span style={{ color: '#eab308' }}>{project.staleSections} stale</span>
            )}
          </div>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          ...
          {activeTab === 'state' && dag && (
            <StateView sections={dag.sections || []} />
          )}

          {activeTab === 'threads' && dag && (
            <ThreadList threads={dag.threads || []} />
          )}

          {activeTab === 'tasks' && dag && (
            <TaskListInline tasks={dag.tasks || []} />
          )}
...
// Inline task list for the Tasks tab (reuses DAG task data)
function TaskListInline({ tasks }: { tasks: EntityNode[] }) {
  // Group by status
  const grouped: Record<string, EntityNode[]> = {}
  for (const task of tasks) {
    const status = task.status.split(' ')[0] // strip priority suffix
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {items.map(task => (
                <div key={task.id} style={{
                  ...
                  borderLeft: `3px solid ${statusColors[status] || '#888'}`,
                }}>
                  <div style={{ fontSize: '0.85rem', fontWeight: 500, color: '#e0e0e0' }}>
                    {task.title}
                  </div>
                  <div style={{ fontSize: '0.7rem', color: '#555', marginTop: '4px', fontFamily: 'monospace' }}>
                    {task.id.slice(0, 8)}
                  </div>
                </div>
              ))}
